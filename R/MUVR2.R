...
#' @export
MUVR2=function(X,Y,ID,nRep=5,nOuter=6,nInner,varRatio=0.75,DA=FALSE,fitness=c('AUROC','MISS','RMSEP'),method=c('PLS','RF'),nCompMax,methParam,ML=FALSE,modReturn=FALSE,logg=FALSE,parallel=TRUE){
  library(pROC)
  library(foreach)
  ...
  reps=foreach(r=1:nRep, .packages=packs, .export=exports) %doVersion% {
    ...
    if (methParam$meanMeth=='arit') varRepMid=round(mean(c(var[minIndex],var[maxIndex])))
    parReturn$varRepMin=varRepMin
    parReturn$varRepMid=varRepMid
    parReturn$varRepMax=varRepMax
