...
plspredict.plsMUVR <-function(object,
  ...
  if (length(dim(newdata)) == 2) {
    if (ncol(newdata) != p)                            ###col numbers are not the same
      stop("'newdata' must be a numeric matrix with ncol = ", p,
           " or a vector of length = ", p, ".")
    if(!identical(colnames(x),colnames(newdata)))      ###col names are not the same
      stop("Mismatch between columns in model and newdata.")
  }

###############################################################################################################################

  if (length(dim(newdata)) == 0) {    ###when there
    if (length(newdata) != p)
      stop("'newdata' must be a numeric matrix with ncol = ", p,
           " or a vector of length = ", p, ".")
    dim(newdata) = c(1, p)                       ##change the vector to matrix
  }
###########################################################################################################
  #-- initialisation des matrices --#
  ncomp = object$ncomp
  a = object$loadings$X                ## row is variables, column is component
  b = object$loadings$Y
  c = object$mat.c                                   ##########################
  ...
  ##- prediction
  y.hat = y.hat2 = array(0, dim = c(nrow(newdata), q, ncomp)) ###newdata obsevations,ncolx,ncomp
...
##############################################################################################################
  variates.x = object$variates$X     ###row is observations, col is components
  ...
  for(h in 1:ncomp){
    ...
    if (q==1) betay[[h]]=(dd[-1]) else betay[[h]]=(dd[-1,])    ###q is ncoly
####################################################################################################################################
#############I don't quite understand this
    W = a[, 1:h,drop=FALSE] %*% solve(t(c[, 1:h,drop=FALSE]) %*% a[, 1:h,drop=FALSE])  ###if AX=B,solve(A,B) is X
...
##############################################################################################################################
  ...
  if (onlyPred) return(invisible(list(predict = y.hat)))
  else return(invisible(list(predict = y.hat,      ###row is new data observation times, column is y col
                             variates = t.pred,    ###row is new data observation times,column is y col
                             B.hat = B.hat,
                             betay=betay)))
}
...
plspredict.plsdaMUVR=function(object,
  ...
  if (onlyPred) return(invisible(list(predict = y.hat)))
  else return(invisible(list(predict = y.hat,
                             variates = t.pred,
                             B.hat = B.hat,
                             betay=betay)))
}
