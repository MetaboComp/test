...
pls <- function(x,
  ...
  if (is.null(ind.names)) {    ###if x name is null, name it as y names
  ...
  mat.t = mat.u = matrix(nrow = n, ncol = ncomp)  ##row is observation,column is component
  mat.a = mat.c = matrix(nrow = p, ncol = ncomp)  ##row is x variable,column is component
  mat.b = mat.d = matrix(nrow = q, ncol = ncomp)  ##row is y variable,column is component

  # Iterate pls components h
  iter=NULL
  for (h in 1:ncomp) {
    #-- initialisation --#
    M = crossprod(x.temp, y.temp) ####x1 dim(4,5),y1 dim(4,2)  crossprod(x1,y1) dim(5,2)
    svd.M = svd(M,
...
##############################################################################################
  ...
  #-- valeurs sortantes --#
  rownames(mat.a) = rownames(mat.c) = x.names    ##same as matrix u
