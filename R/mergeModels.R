#' Merge two MUVR class object that use regression
...
mergeModels=function(MV1,MV2) {
  ...
  if(any(class(MV2)=='Multilevel')|any(class(MV2)=='Classification')) {
      cat('\nNot yet supported')
      stop()
  }
#####name in data
...
####save everything of MV1 , whichs indata  is identical as MV2
  DA=MV1$inData$DA
  PLS=MV1$inData$method=='PLS'      ###if used pls, PLS=T , if RF.PLS=F
####
  yP=MV1$yPred
