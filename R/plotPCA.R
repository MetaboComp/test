...
##for color variable
  if (missing(colVar)) {
    ...
    cols=colorRampPalette(c('blue','yellow','red'))(40)   ####divide color to smaller color
    col=cols[cut(colVar,40)]               ################cut color to 40 intervals and it is transformed to a factor variable now
...
##For symbol variable
  if (missing(symbVar)) {
    ...
    symbLeg=FALSE   ####Categorical/discrete variable for multiple plot symbols
  } else {
...
##For png
  ...
  if (plotPNG) png(filename=file,
                   width=1024,    ### The width of the device.
                   height=1024,   ### The height of the device.
                   pointsize=36)  ### The default pointsize of plotted text, interpreted as big points (1/72 inch) at res ppi.


  if (main=='') par(mar=c(4,4,0,0)+.5)   ######c(bottom, left, top, right)
  else par(mar=c(4,4,2,0)+.5)

  pcVar=summary(pca)$importance[2,]    ####proportion of variabce

  xlab=paste('PC',PC1,' (R2X=',signif(pcVar[PC1],3),')',sep='')   ###PC1 label proportion of variance
  ylab=paste('PC',PC2,' (R2X=',signif(pcVar[PC2],3),')',sep='')   ###PC2 label proportion of variance


  plot(pca$x[,c(PC1,PC2)],    ##observations
       main=main,
       col=col,                 ### repeated color is sequenced by observations
       xlab=xlab,
       ylab=ylab,
       pch=pch) # scoreplot

  if (colLeg) legend('topleft',
                     col=c('blue','yellow','red'),
                     legend=c('low','mid','high'),
                     pch=1)                            ###hollow circle

  if (symbLeg) legend('bottomleft',
                      col=1,
