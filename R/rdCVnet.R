...
#' @export
rdCVnet <- function(X, Y, ID, alow=1e-5, ahigh=1, astep=11, alog=TRUE, nRep=5, nOuter=6, nInner, NZV=TRUE,
                    DA=FALSE, fitness=c('AUROC', 'MISS', 'BER', 'RMSEP'), methParam,
                    ML=FALSE, modReturn=FALSE, parallel=TRUE, ...){
  ...
  if(DA) methParam$family <- 'multinomial'

  ## Store indata in list for later model return
  InData <- list(X=X, Y=Y, ID=ID, alow=alow, ahigh=ahigh, astep=astep, nRep=nRep, nOuter=nOuter, nInner=nInner,
                 DA=DA, fitness=fitness, methParam=methParam, ML=ML, parallel=parallel)
  ...
  if (DA) {
    ...
    yPredSeg <- matrix(nrow=length(Y), ncol=length(levels(Y)), dimnames=list(ID,levels(Y)))
  } else {
    # Predictions per repetition
  ...
  if (alog) {
    avect <- seq(from=log10(alow), to=log10(ahigh), length.out=astep) # logarithmic
    avect <- 10^avect
  } else avect <- seq(from=alow, to=ahigh, length.out=astep) # Arithmetic
  ...
  reps <- foreach(r=1:nRep, .packages=packs, .export=exports) %doVersion% {
    ...
    if(DA) {
      coefSeg <- array(0, dim=c(nVar0, length(levels(Y)), nOuter), dimnames = list(colnames(X), levels(Y), paste0('Segment',1:nOuter)))
    } else {
      coefSeg <- matrix(0, nrow=nVar0, ncol=nOuter)
      rownames(coefSeg) <- colnames(X)
      colnames(coefSeg) <- paste0('Segment',1:nOuter)
    }
    alphaSeg <- lambdaSeg <- nonZeroSeg <- numeric(nOuter)
    if(DA) nonZeroSegClass <- matrix(nrow=nOuter, ncol=length(levels(Y)))
    VALSeg <- matrix(nrow=nOuter, ncol=astep)
    ...
    for (i in 1:nOuter) {
      ...
      VALInner <- matrix(nrow=methParam$nRepInner,ncol=astep)
      ...
      for (j in 1:methParam$nRepInner) {
        ...
        foldID <- foldVector(foldList = allVal, ID = idIn)
        ...
        for (count in 1:astep) {  # Build models with successively fewer variables. Quality metric = number of missclassifications for Validation set
          ...
          # count <- count + 1
          alpha <- avect[count]
          suppressWarnings(inMod <- cv.glmnet(x = xIn, y = yIn, alpha=alpha, family=methParam$family, foldid = foldID))
      ...
      suppressWarnings(inMod <- cv.glmnet(x = xIn, y = yIn, alpha=alphaSeg[i], family=methParam$family, foldid = foldID))
      if (modReturn) outMod[[i]] <- inMod
      whichMin <- which.min(inMod$cvm)
      lambdaSeg[i] <- inMod$lambda.min
      if(DA) {
        coefs <- coef(inMod, s = lambdaSeg[i]) %>% sapply(., as.matrix)
        coefs <- coefs[-1,]
        rownames(coefs) <- colnames(X)
        coefSeg[,,i] <- coefs
        nonZeroSeg[i] <- apply(coefs,1,function(x) any(x!=0)) %>% sum
        nonZeroSegClass[i,] <- apply(coefs,2,function(x) sum(x!=0))
        varSegClass[[i]] <- lapply(as.data.frame(coefs),function(x, nm=rownames(coefs)) {
          whichNZ <- which(x!=0)
          return(nm[whichNZ])
        })
        varSeg[[i]] <- unlist(varSegClass[[i]]) %>% unique
        varSeg[[i]] <- varSeg[[i]][order(match(varSeg[[i]],colnames(X)))] # Resort according to order in X
        yPredSeg[testIndex,] <- predict(inMod, newx = xTest, s = lambdaSeg[i], type = 'response')
      } else {
        nonZeroSeg[i] <- inMod$nzero[whichMin]
        coefSeg[,i] <- as.matrix(coef(inMod, s=lambdaSeg[i]))[-1,,drop=F]
        varSeg[[i]] <- names(coefSeg[,i][coefSeg[,i]!=0])
        yPredSeg[testIndex] <- predict(inMod, newx = xTest, s = lambdaSeg[i])
      }
      # plot(yTest, yPred)
    }
  ...
  if (DA) {
    ...
    yPredRep <- array(dim=c(length(Y), length(levels(Y)), nRep),
                      dimnames=list(ID, levels(Y), paste('Rep', 1:nRep,sep='')))
  } else {
    # Predictions for all repetitions
    yPredRep <- matrix(nrow=length(Y), ncol=nRep, dimnames=list(ID, paste('Rep', 1:nRep,sep='')))
  }
  # Allocate validation and parameter variables
  alphaRep <- lambdaRep <- nonZeroRep <-
    matrix(nrow=nRep, ncol=nOuter, dimnames=list(paste('repetition', 1:nRep, sep=''), paste('segment', 1:nOuter,sep='')))
  if(DA) coefRep=list() else coefRep <- array(0, dim=c(nVar0, nOuter, nRep), dimnames=list(colnames(X), paste0('Segment',1:nOuter), paste0('Rep',1:nRep)))
  VALRep <- array(dim=c(nOuter, astep, nRep), dimnames=list(paste('outSeg', 1:nOuter,paste=''), avect, paste(rep('rep', nRep), 1:nRep,sep='')))
  varRep <- list()
  if(DA) {
    varsClassRep <- list()
    nonZeroClassRep <- array(dim=c(nOuter, length(levels(Y)), nRep), dimnames=list(paste('outSeg', 1:nOuter,paste=''), levels(Y), paste(rep('rep', nRep), 1:nRep,sep='')))
  }
  ...
  for (r in 1:nRep) {
    if (DA) yPredRep[,,r] <- reps[[r]]$yPred else yPredRep[,r] <- reps[[r]]$yPred
    ...
    if(DA) coefRep[[r]] <- reps[[r]]$coef else coefRep[,,r] <- reps[[r]]$coef
    VALRep[,,r] <- reps[[r]]$VAL
  ...
  # Average predictions
  if (DA) {
    yPred <- apply(yPredRep,c(1,2),mean)
  } else {
    yPred <- apply(yPredRep,1,mean)[1:nrow(X)]
  }
  ...
  if (DA) {
    ...
    yClass <- apply(yPred, 1, function(x) levels(Y)[which.max(x)])
    miss <- sum(yClass!=Y)
    auc <- numeric(length(levels(Y)))
    names(auc) <- levels(Y)
    for (cl in 1:length(levels(Y))) auc[cl] <- roc(Y==(levels(Y)[cl]),yPred[,cl], quiet = TRUE)$auc
    # # Report
    modelReturn$yClass <- yClass
    modelReturn$miss <- miss
    modelReturn$auc <- auc
  } else if (ML) {
    modelReturn$yClass <- ifelse(yPred>0,1,-1)
    modelReturn$miss <- sum(yClass!=Y)
    modelReturn$auc <- roc(Y,yPred, quiet=TRUE)$auc
    names(modelReturn$yClass)=paste(1:nSamp,ID,sep='_ID')
  }
  # Calculate overall nVar
  VAL <- apply(VALRep,2,mean)
  ...
  if (DA) {
    modelReturn$varClassRep <- sapply(varsClassRep, function(x) unlist(x) %>% table %>% sort(., decreasing = T))
  ...
  # Fit-predict model
  suppressWarnings(cvFit <- cv.glmnet(X, Y, alpha=modelReturn$alpha, family = methParam$family))
  suppressWarnings(fitPredict <- glmnet(X, Y, alpha=modelReturn$alpha, lambda = cvFit$lambda.min, family = methParam$family))
  yFit <- predict(fitPredict, newx = X, type='response')
  ...
  class(modelReturn) <- c('MUVR',ifelse(DA,'Classification',ifelse(ML,'Multilevel','Regression')))
  return(modelReturn)
}
