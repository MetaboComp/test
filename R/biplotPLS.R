...
#########################################################################a loading pplot
  ...
  mtext('Loadings',
        ...
        line=3,
        cex=cex)
  abline(h=0,   ###add axis at origin point
         v=0,
         lty=2,    ##line type
         col='grey')
...
#####label on loading
  if(labPlLo) text(1.1*loads,   ###numeric vectors of coordinates where the text labels should be written. If the length of x and y differs, the shorter one is recycled.
                   rownames(loads),   ###a character vector or expression specifying the text to be written.
                   cex=.7*cex,   ##numeric character expansion factor; multiplied by par("cex") yields the final character size. NULL and NA are equivalent to 1.0.
...
#################score plot
  ...
  plot(scores,      ####  Scores with x and y value
       ...
       pch=pchSc,   ###pch for observation scores (only if xCol omitted)
       ylim=rSc,
       xlim=rSc,
       bty='l',
  ...
  if (legPlot) {    ###do legend plot or not
    if (missing(xCol)) {
      legend('topright',
             legend=unique(colScLeg),   ###for score legend
             pch=unique(pchSc),          ###Plotting character for observation scores
      ...
      legend('topright',
             legend=signif(unik,3), ##round it to 3 digits
             fill=cols,  ###if specified, this argument will cause boxes filled with the specified colors (or shaded in the specified colors) to appear beside the legend text.
             bty='n')
    }
  }
}
