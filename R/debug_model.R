debug_model <- function(X,
  ...
  if (any(is.na(X)) |
      any(is.na(Y))){stop('No missing values allowed in X or Y data.')}
...
#######################################################################################################################################
                    ...
                    if (fitness == 'AUROC')
                      fitRankRep <-
                      -fitRankRep # AUROC fitness (higher is better) goes in opposite direction of all other metrics (lower is better)
                    fitRankRep <-
                    ...
                    if (all(is.nan(fitRankRep)))
                      fitRankRep <-
                      rep(0, cnt) # If all VAL have same value -> reset all fitRankRep to 0
  ...
  if (all(is.nan(fitRankAll)))
    fitRankAll <-
    rep(0, cnt) # If all VAL have same value -> reset all fitRankAll to 0
