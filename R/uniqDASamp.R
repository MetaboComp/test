...
uniqDASamp <- function(Y,
  ...
  for (grp in 1:groups) {
    ...
    groupList[[grp]] <- vectSamp(groupID,n=folds)  # Draw random samples within group, smallest length in the beginning
  }
  masterList <- groupList[[1]] # Add 1st class samplingintofolds to 'Master' sample of all groups
