...
vip <- function(object) {
  ...
  cor2 = cor(object$Y,             ##row is observation,col is  Y variables
  ...
  cor2 = as.matrix(cor2, nrow = q)   ### with nrow of Y variables numbers and ncol of component
  VIP = matrix(0, nrow = p, ncol = H)   ###row is X variables, col is component
  VIP[, 1] = W[, 1]^2   ##the first component for each variable square,  length is the number of variables
  if (H > 1) {                ##H is number of components of X
    for (h in 2:H) {
      if (q == 1) {           ##q is number of variables of Y. When there is only one variable in Y
        Rd = cor2[, 1:h]      ##with nrow of Y variables numbers and ncol of component
      } else {
        Rd = colSums(cor2[, 1:h])   ##Add all row in each col together. There is one number for each component
      }
