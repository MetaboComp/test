...
#' @export
preProcess=function(X,offset=0,
                    zeroOffset=0,
                    trans='none',
                    center='none',
                    scale='none') {
...
#[1] "gaussian"     "rectangular"  "epanechnikov"
  ...
  if (length(center)!=nVar) {
    ...
    cat('\nCenter:',center)
    if (center=='mean') center=TRUE else center=FALSE    ###what does this mean
...
##
  ...
  if (length(scale)!=nVar) {
    scale=match.arg(scale,c('UV','Pareto','none'))  ###unit variance
    cat('\nScale:',scale)
    if (scale=='UV') scale=TRUE
    else if (scale=='none') scale=FALSE
    else scale=apply(X,2,function(x) sqrt(sd(x)))   ####When Pareto scaling
