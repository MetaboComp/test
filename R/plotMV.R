...
#####################
  ...
  if (any(class(MUVRclassObject) == 'Regression')) {
    ...
    # Y-limits
    if (is.null(ylim)) {ylim <- range(YPR)}  ###returns a vector containing the minimum and maximum of all the given arguments
    # Plot Y-predicted per repetition in grey
    matplot(Y,                                      #####X axis 112 observations
            YPR,                                   ####Y axis 112 obsevations each of them have 7 repetitions
            pch = 20,
            xlab = 'Original Y',
            ylab = 'Predicted Y',
            col = 'grey',
    ...
    # Add legend
    legend('topleft',
           legend = c(paste('Model R2 =', signif(MUVRclassObject$fitMetric$R2[modNum], 3)),
                      paste('Model Q2 =', signif(MUVRclassObject$fitMetric$Q2[modNum], 3))),
    ...
    YPR <- MUVRclassObject$yPredPerRep[[modNum]]
      ...
      YP <- MUVRclassObject$yPred     ####The probability that belongs to each class
      # Y-predicted per repetition
      YPR <- MUVRclassObject$yPredPerRep
    ...
    ##
    classNudge <- 0.2 * ((classes - mean(classes)) / (mean(classes) - 1))  ###create distance between different classification on x axis
    ...
    axis(1,                     ###1=below, 2=left, 3=above and 4=right.
         at = 1:length(Y),      ###the points at which tick-marks are to be drawn.
         labels = sampLabels,
    ...
    for(cl in classes) {
      ...
      matpoints((1:nSamp) + classNudge[cl],
                YPR[,cl,],                                ##For all the observations for all repetition in each class
                pch = 20,
                col = factCols[cl],
                cex = 0.5)
      # Y-pred overall
    ...
    wrongClass <- as.numeric(Y[whichWrong])      ##Transform to the corresponding real class number of the miss classification ones
...
##I didn't figure out how to add legend here for classification problem
    ...
    legend(
           ...
           pch = c(rep(16,length(classes)), 1),  ###the plotting symbols appearing in the legend, a
           col = c(factCols,1),
           cex = 0.8,
